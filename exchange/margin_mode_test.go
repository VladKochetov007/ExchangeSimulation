...
func setupMarginModeExchange() (*Exchange, *MarginModeManager) {
	...
	return ex, NewMarginModeManager(ex)
}

func TestSetMarginMode_Success(t *testing.T) {
	ex, mgr := setupMarginModeExchange()
...
func TestSetMarginMode_UnknownClientReturnsError(t *testing.T) {
	_, mgr := setupMarginModeExchange()

	err := mgr.SetMarginMode(999, IsolatedMargin)
	if err == nil {
...
func TestSetMarginMode_FailsWithOpenPositions(t *testing.T) {
	...
	err := mgr.SetMarginMode(1, IsolatedMargin)
	if err == nil {
...
func TestAllocateCollateralToPosition_Success(t *testing.T) {
	ex, mgr := setupMarginModeExchange()
	_ = mgr.SetMarginMode(1, IsolatedMargin)

	perpBefore := ex.Clients[1].PerpBalances["USD"]
	amount := USDAmount(5_000)

	if err := mgr.AllocateCollateralToPosition(1, "BTC-PERP", "USD", amount); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestAllocateCollateralToPosition_WrongModeReturnsError(t *testing.T) {
	...
	err := mgr.AllocateCollateralToPosition(1, "BTC-PERP", "USD", USDAmount(1_000))
	if err == nil {
...
func TestAllocateCollateralToPosition_InsufficientBalanceReturnsError(t *testing.T) {
	...
	err := mgr.AllocateCollateralToPosition(1, "BTC-PERP", "USD", USDAmount(999_999_999))
	if err == nil {
...
func TestReleaseCollateralFromPosition_Success(t *testing.T) {
	...
	amount := USDAmount(5_000)
	_ = mgr.AllocateCollateralToPosition(1, "BTC-PERP", "USD", amount)
	perpAfterAlloc := ex.Clients[1].PerpBalances["USD"]

	if err := mgr.ReleaseCollateralFromPosition(1, "BTC-PERP", "USD", amount); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestReleaseCollateralFromPosition_InsufficientCollateralReturnsError(t *testing.T) {
	...
	err := mgr.ReleaseCollateralFromPosition(1, "BTC-PERP", "USD", USDAmount(1_000))
	if err == nil {
