...
func (a *ExchangeAutomation) updateAllPerpPrices() {
	...
	// Now call Price without holding e.mu.
	...
	for _, c := range candidates {
		indexPrice := a.indexProvider.Price(c.symbol)
		if indexPrice == 0 {
			continue
		}
	...
	for _, client := range a.exchange.Clients {
		for asset, borrowed := range client.Borrowed {
			...
			if interest > 0 {
				...
				logBalanceChange(a.exchange, timestamp, client.ID, "", "interest_charge", []BalanceDelta{
...
func (a *ExchangeAutomation) CheckLiquidations(symbol string, perp *PerpFutures, markPrice int64) {
	...
	for clientID, client := range a.exchange.Clients {
		...
		if marginRatio < perp.MaintenanceMarginRate {
			...
			a.liquidate(clientID, client, symbol, pos, perp, timestamp)
...
func (a *ExchangeAutomation) liquidate(clientID uint64, client *Client, symbol string, pos *Position, perp *PerpFutures, timestamp int64) {
	...
	if a.exchange.BorrowingMgr != nil {
		...
		if borrowed > 0 {
			...
			if availableForRepay > 0 {
				...
				logBalanceChange(a.exchange, timestamp, clientID, symbol, "liquidation_repay", []BalanceDelta{
	...
	if remainingEquity < 0 {
		...
		logBalanceChange(a.exchange, timestamp, clientID, symbol, "liquidation_deficit", []BalanceDelta{
		...
		logBalanceChange(a.exchange, timestamp, clientID, symbol, "liquidation_surplus", []BalanceDelta{
			reservedPerpDelta(perp.QuoteAsset(), oldReserved, 0),
		})
	}
