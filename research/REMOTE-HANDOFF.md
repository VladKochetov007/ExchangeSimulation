...
- The canonical Git endpoint is the tip of
  `autoresearch/ffa-ecology-gen0` after the final handoff commit; verify it
  with `git rev-parse HEAD` (the pushed tip at transfer is reported in the
  transfer receipt, not inferred from this prose).
...
1. `d38575d` — finalized and pushed the transfer handoff.
2. `2008daa` — recorded the compact private-archive checksum and contents.
3. `cf0d83f` — recorded the compact private-archive scope.
4. `deb00f9` — moved the three untracked research notes under `research/`
   so the notes travel with the public source history.
5. `d6d99e0` — added this tracked remote handoff.
6. `30176e0` — hardened integrated long-run provenance, registered GOMAXPROCS
   and 24-hour identity, copied-config execution, and atomic completion
   status.
7. `5af4573` — validated clean binary provenance for long-run runs.
