...
v2_r2_require_output_root "$output_root" || fail "scorer root is not the canonical R2 evidence root"
...
for cell in dev-607 dev-613 dev-617; do
	...
	v2_r2_require_cell_path "$cell_dir" || fail "scorer cell is outside the canonical R2 root or is symlinked: $cell"
	MVANALYZE_BIN="$analyzer" "$root_dir/scripts/verify-v2-integrated-longrun-r2-cell.sh" "$cell_dir" >/dev/null ||
	...
	jq -e '.schema_version == 1 and .contract == "v2-integrated-longrun-r2-candidate-v2" and
	...
	jq -e '.schema_version == 1 and .result.contract == "v2-integrated-longrun-r2-candidate-v2" and
		...
		.result.predicates.calendar_behavior_attested == true and .result.calendar.contract == "calendar-audit-v2" and
