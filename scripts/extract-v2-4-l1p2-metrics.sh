#!/usr/bin/env bash
# Extract V2-4 L1-P2's full preregistered evidence contract. This script never
# prunes raw evidence, including a cell whose causal prediction is falsified.
set -euo pipefail

root_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
artifact_dir="$root_dir/research/artifacts/v2-4-l1p2"
...
for arm in A B C D; do
	case "$arm" in
		A) expected_liability_phase=0; expected_noise_phase=0 ;;
		B) expected_liability_phase=1000000000; expected_noise_phase=0 ;;
		C) expected_liability_phase=0; expected_noise_phase=1000000000 ;;
		D) expected_liability_phase=1000000000; expected_noise_phase=1000000000 ;;
	esac
	for seed in 101 103; do
		cell="$artifact_dir/$arm/seed-$seed"
		if [[ ! -s "$cell/greeks.json" || ! -s "$cell/latency.json" ]]; then
			echo "incomplete V2-4 L1-P2 cell (needs final greeks.json + latency.json): $cell" >&2
			exit 1
		fi
		if [[ ! -s "$cell/run-config.json" || ! -s "$cell/run-metadata.json" ]]; then
			echo "missing L1-P2 provenance input: $cell" >&2
			exit 1
		fi

		expected_noise_actors=$(jq '.noise_trader_count * (.venue_ids | length)' "$cell/run-config.json")
		...
		# A cell can be scientifically negative, but it cannot be scored if any
		# evidence contract is absent, malformed, or timing-inactive.
		jq -e --argjson phase "$expected_liability_phase" '
			...
			.result.non_reducing_fills == 0
		' "$cell/liabilityhedger.json" >/dev/null
		jq -e --argjson phase "$expected_noise_phase" --argjson actors "$expected_noise_actors" '
		...
		jq -n \
			--arg arm "$arm" \
			--argjson seed "$seed" \
			--argjson liability_phase "$expected_liability_phase" \
			--argjson noise_phase "$expected_noise_phase" \
			...
			{
			  schema_version: 1,
			  experiment_id: "V2-4-L1-P2",
			  arm: $arm,
			  seed: $seed,
			  liability_decision_phase_offset_nanos: $liability_phase,
			  noise_flow_decision_phase_offset_nanos: $noise_phase,
		...
		jq -n \
			...
			'{
			  ...
			  required_artifacts: [
			    "observationreceipts.json", "evidenceartifacthash.json", "liabilityhedger.json", "noiseflowphase.json",
			    "viability.json", "viability-after-warmup.json", "l1p2-activity-gate.json"
			  ],
			  raw_log_policy: "retained; no L1-P2 raw evidence is prunable from this script"
			}' >"$cell/analysis-metadata.json"
		echo "extracted L1-P2 $arm/$seed"
	done
done
