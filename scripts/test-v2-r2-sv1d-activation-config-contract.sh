...
jq -n --arg contract "$v2_r2_sv1d_capacity_attestation_contract" --arg revision "$capacity_shape_revision" \
	...
	'{schema_version:1,contract:$contract,measurement:"synthetic_24h_binary_evidence_capacity_probe",capacity_only:true,
	 ...
	 capacity_binary:{path:"bin/evscapacity",sha256:$hash},target_config:{path:"research/configs/v2-r2-sv1d-activation/activation-659-treatment.json",sha256:$hash},
	 review:{path:"research/reviews/sv1d-capacity-review.json",sha256:$hash},workload:{profile:$profile,seed:$seed,event_count:$event_count,book_delta_events:$book_events,balance_change_events:$balance_events,opaque_scientific_events:$opaque_events,start_nano:$start,end_nano:$end,horizon:"24h"},
