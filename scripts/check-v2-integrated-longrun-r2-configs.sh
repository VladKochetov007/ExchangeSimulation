...
for cell in dev-607 dev-613 dev-617 holdout-619 holdout-631 holdout-641; do
	...
	jq -e --arg cell "$cell" --argjson seed "$seed" --argjson calendar "$calendar" \
		'.seed == $seed and
	...
	if [[ "$(jq -s '.[0] == .[1]' \
		<(jq 'del(.seed, .experiment_id, .hypothesis_id, .description, .dated_future_delivery_fee_policy, .short_future_tenor, .long_future_tenor, .short_option_tenor, .long_option_tenor, .r2_expiry_calendar)' "$source_full") \
		<(jq 'del(.seed, .experiment_id, .hypothesis_id, .description, .dated_future_delivery_fee_policy, .short_future_tenor, .long_future_tenor, .short_option_tenor, .long_option_tenor, .r2_expiry_calendar)' "$file"))" != true ]]; then
		fail "non-calendar economic/config drift in $file"
	fi
done

none="$config_dir/dev-607-none.json"
jq -e --argjson calendar "$calendar" \
	'.seed == 607 and .experiment_id == "v2-integrated-longrun-r2-dev-607-none" and
	 .hypothesis_id == "V2-INTEGRATED-LONG-R2-CANDIDATE-PARITY" and .log_mode == "none" and
...
if [[ "$(jq -s '.[0] == .[1]' \
	<(jq 'del(.seed, .experiment_id, .hypothesis_id, .description, .dated_future_delivery_fee_policy, .short_future_tenor, .long_future_tenor, .short_option_tenor, .long_option_tenor, .r2_expiry_calendar)' "$source_none") \
	<(jq 'del(.seed, .experiment_id, .hypothesis_id, .description, .dated_future_delivery_fee_policy, .short_future_tenor, .long_future_tenor, .short_option_tenor, .long_option_tenor, .r2_expiry_calendar)' "$none"))" != true ]]; then
