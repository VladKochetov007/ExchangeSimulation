#!/usr/bin/env bash
# Run one immutable V2-4 L1-P3 holdout cell. Completion is defined only by the
# final greeks.json and latency.json sidecars; partial output is never evidence.
set -euo pipefail

if [[ $# -lt 2 || $# -gt 3 ]]; then
	echo "usage: $0 A|B|C|D 107|109|113 [multivenue-binary]" >&2
	exit 2
fi

arm=$1
seed=$2
case "$arm" in A|B|C|D) ;; *) echo "unknown L1-P3 arm: $arm" >&2; exit 2 ;; esac
case "$seed" in 107|109|113) ;; *) echo "unregistered L1-P3 seed: $seed" >&2; exit 2 ;; esac

root_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
config="$root_dir/research/configs/v2-4-l1p3/$arm-$seed.json"
output="$root_dir/research/artifacts/v2-4-l1p3/$arm/seed-$seed"
...
if [[ -e "$output" ]]; then
	echo "refusing to overwrite L1-P3 evidence directory: $output" >&2
	exit 1
fi
...
jq -n \
	...
	'{
	  ...
	  raw_log_policy: "retain until every L1-P3 receipt, evidence-artifact, liability-policy/phase, noise-phase, exact-gap, fill-transition, and non-collapse contract passes review"
