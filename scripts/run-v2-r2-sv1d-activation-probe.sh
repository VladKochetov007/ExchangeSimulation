...
v2_r2_sv1d_require_capacity_attestation "$capacity_attestation" "$head_revision" "$capacity_binary_sha256" \
...
write_invalid_provenance() {
	...
	jq -n --arg contract "$v2_r2_sv1_activation_contract" --arg candidate "$v2_r2_sv1_candidate_id" \
		...
		--arg capacity_path "$capacity_attestation" --arg capacity_sha256 "$capacity_attestation_sha256" \
			--arg capacity_binary_path "$capacity_binary" --arg capacity_binary_sha256 "$capacity_binary_sha256" \
		 ...
		 status: $status, activation_satisfied: false, holdouts_consumed: false, reason: $reason,
			 ...
			 capacity: {path: $capacity_path, sha256: $capacity_sha256, binary_path: $capacity_binary_path,
			 binary_sha256: $capacity_binary_sha256, config_sha256: $capacity_config_sha256, contract: $capacity_contract}}' \
		>"$tmp"
	mv -- "$tmp" "$path"
}
