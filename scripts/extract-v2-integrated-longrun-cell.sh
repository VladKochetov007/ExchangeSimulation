	...
	'def r($x): $x[0].result;
	 ...
	 {schema_version: 1, contract: $contract,
		...
		predicates: {
			...
			conservation: (count($conservation; "flows") > 0 and count($conservation; "identities") > 0 and count($conservation; "venue_identities") > 0 and count($conservation; "delta_consistency.checked") > 0 and zero($conservation; "delta_consistency.mismatched") and zero($conservation; "delta_consistency.chain_broken") and zero($conservation; "delta_consistency.decode_failures") and zero($conservation; "delta_consistency.malformed_venue_records") and zero($conservation; "delta_consistency.malformed_fee_records") and zero($conservation; "delta_consistency.arithmetic_failures") and residuals_within(r($conservation).identities) and residuals_within(r($conservation).venue_identities)),
