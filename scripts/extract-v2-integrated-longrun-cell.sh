	...
	'def r($x): $x[0].result;
	 ...
	 {schema_version: 1, contract: $contract,
		...
		predicates: {
			...
			conservation: (count($conservation; "flows") > 0 and count($conservation; "identities") > 0 and count($conservation; "venue_identities") > 0 and count($conservation; "delta_consistency.checked") > 0 and zero($conservation; "delta_consistency.mismatched") and zero($conservation; "delta_consistency.chain_broken") and zero($conservation; "delta_consistency.decode_failures") and zero($conservation; "delta_consistency.malformed_venue_records") and zero($conservation; "delta_consistency.malformed_fee_records") and zero($conservation; "delta_consistency.venue_balance_mismatches") and zero($conservation; "delta_consistency.fee_revenue_mismatches") and zero($conservation; "delta_consistency.arithmetic_failures") and residuals_within(r($conservation).identities) and residuals_within(r($conservation).venue_identities)),
			...
			derivatives: (count($derivatives; "funding") > 0 and field_zeroes($derivatives; ["funding_broken", "funding_sign_wrong", "funding_misdirected", "funding_undirected", "funding_duplicate_payments", "funding_missing_rates", "funding_missing_settlements", "funding_timing_failures", "funding_evidence_failures", "funding_arithmetic_failures", "exercise_broken", "exercise_timing_failures", "exercise_evidence_failures", "holders_mispaid", "worthless_paid", "exercise_arithmetic_failures", "exercise_missing_payouts", "exercise_extra_payouts", "exercise_duplicate_payouts", "exercise_unknown_payouts"])),
