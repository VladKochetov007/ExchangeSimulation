#!/usr/bin/env bash
# Render the P3 R1 replacement cells after attempt 0's evidence-only
# invalidation. The economic treatment remains the original 0/5,000 bps pair.
set -euo pipefail

root_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
parent_dir="$root_dir/research/configs/v2-3-p1"
output_dir="$root_dir/research/configs/v2-3-p3-r1"

if ! command -v jq >/dev/null; then
	echo "jq is required to render V2-3 P3 R1 configs" >&2
	exit 1
fi
...
render() {
	...
	jq \
		...
		--argjson seed "$seed" \
		'
		.experiment_id = ("v2-3-p3-r1-" + $arm + "-seed-" + ($seed|tostring)) |
		.hypothesis_id = "V2-3-P3-R1" |
		.date = "2026-08-24" |
		.status = "preregistered" |
		.description = "Five-minute ABC-PERP confirmed passive-quote replenishment R1 screen; attempt 0 evidence invalid, P0-C and P1-B fixed." |
		.seed = $seed |
	...
	if ! diff -u \
		...
		echo "P3 R1 config contains an undeclared change: $output" >&2
		exit 1
	fi
}
...
for seed in 101 103; do
	if ! diff -u \
		...
		echo "P3 R1 A/B configs differ outside declared replenishment threshold: seed $seed" >&2
		exit 1
	fi
done
