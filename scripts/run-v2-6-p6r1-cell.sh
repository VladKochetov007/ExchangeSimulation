#!/usr/bin/env bash
# Run one immutable V2-6 P6-R1 cross-asset-mark cell.  Only final
# greeks.json and latency.json are completion sentinels; an exited process or
# partial directory is never evidence.
set -euo pipefail
...
stage=$1
seed=$2
case "$stage" in O0|O1|O2|O3|O4) ;; *) echo "unknown P6-R1 stage: $stage" >&2; exit 2 ;; esac
case "$seed" in 211|213|223|227|229) ;; *) echo "unregistered P6-R1 seed: $seed" >&2; exit 2 ;; esac
if [[ "$seed" == 223 || "$seed" == 227 || "$seed" == 229 ]]; then
	if [[ "${P6R1_ALLOW_HOLDOUT:-}" != 1 ]]; then
		echo "refusing P6-R1 holdout before development promotion: set P6R1_ALLOW_HOLDOUT=1" >&2
		exit 1
	fi
fi

root_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
config="$root_dir/research/configs/v2-6-p6r1/$stage-$seed.json"
output="$root_dir/research/artifacts/v2-6-p6r1/$stage-$seed"
binary=${3:-"$root_dir/bin/multivenue"}

if [[ ! -s "$config" || ! -x "$binary" ]]; then
	echo "missing P6-R1 config or executable: $config / $binary" >&2
	exit 1
fi
if [[ -e "$output" ]]; then
	echo "refusing to overwrite P6-R1 evidence directory: $output" >&2
	exit 1
fi
...
jq -n \
	...
	'{
	  ...
	  raw_log_policy: "retain until every registered P6-R1 evidence contract passes"
...
if [[ ! -s "$output/greeks.json" || ! -s "$output/latency.json" ]]; then
	echo "P6-R1 process exited without both completion sentinels: $output" >&2
	exit 1
fi
echo "completed P6-R1 cell: $output"
