#!/usr/bin/env bash
# Extract V2-4 L1-P's complete preregistered evidence contract. Raw evidence
# is retained irrespective of a scientific or evidence-integrity result.
set -euo pipefail

root_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
artifact_dir="$root_dir/research/artifacts/v2-4-l1p"
...
for arm in P0 P1; do
	...
	for seed in 101 103; do
		...
		# Evidence integrity is a hard extraction condition. The CDF/USD floor is
		# retained as a scientific outcome below, never as an excuse to discard a
		# valid but phase-sensitive cell.
		jq -e --argjson phase "$expected_phase" '
		...
		jq -n \
			...
			{
			  schema_version: 1,
			  experiment_id: "V2-4-L1-P",
			  arm: $arm,
			  seed: $seed,
			  decision_phase_offset_nanos: $phase,
		...
		jq -n \
			...
			'{
			  ...
			  required_artifacts: [
			    ...
			    "viability.json", "viability-after-warmup.json", "l1p-activity-gate.json"
			  ],
			  raw_log_policy: "retained; no L1-P raw evidence is prunable from this script"
			}' >"$cell/analysis-metadata.json"
		echo "extracted L1-P $arm/$seed"
	done
done
