#!/usr/bin/env bash
# Run one immutable V2-3 P2 local inventory-rebalance cell. Existing evidence
# is never overwritten: a partial directory is not transformed into evidence.
set -euo pipefail
...
arm=$1
seed=$2
case "$arm" in A|B) ;; *) echo "unknown P2 arm: $arm" >&2; exit 2 ;; esac
case "$seed" in 101|103) ;; *) echo "unregistered P2 seed: $seed" >&2; exit 2 ;; esac

root_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
config="$root_dir/research/configs/v2-3-p2/$arm-$seed.json"
output="$root_dir/research/artifacts/v2-3-p2/$arm/seed-$seed"
...
if [[ -e "$output" ]]; then
	echo "refusing to overwrite P2 evidence directory: $output" >&2
	exit 1
fi
...
jq -n \
	...
	'{
	  ...
	  raw_log_policy: "retain until every P2 policy, receipt, execution, fill-transition, inventory, and viability artifact passes review"
