#!/usr/bin/env bash
# Run one immutable V2-3 P3 passive-replenishment cell. Existing evidence is
# never overwritten; a failed/partial directory is historical, not reusable.
set -euo pipefail
...
arm=$1
seed=$2
case "$arm" in A|B) ;; *) echo "unknown P3 arm: $arm" >&2; exit 2;; esac
...
if [[ -e "$output" ]]; then
	echo "refusing to overwrite P3 evidence directory: $output" >&2
	exit 1
fi
...
jq -n \
	...
	'{
	  ...
	  raw_log_policy: "retain until every P3 activation, lifecycle, evidence, and viability artifact passes review"
