	...
	"$(v2_r2_calendar_listing_timeline_jq_definition)"'def r($x): $x[0].result;
	 ...
	 {schema_version: 1, contract: $contract,
		...
		predicates: {
			...
			order_lifecycle: (r($orderlifecycle).unknown_fills == r($orderlifecycle).liquidation_fills and field_zeroes($orderlifecycle; ["unknown_cancellations", "duplicate_acceptances", "duplicate_terminals", "fills_after_terminal", "fill_quantity_mismatches", "cancel_quantity_mismatches", "client_mismatches", "unlinked_fills", "liquidation_identity_failures", "missing_forced_fills", "missing_immediate_terminal", "malformed_accepted_records", "malformed_fill_records", "malformed_cancel_records", "malformed_liquidation_records"])),
			...
			liquidations: (zero($liquidations; "invalid_liquidations") and zero($liquidations; "position_path_missing") and zero($liquidations; "position_path_failures") and zero($liquidations; "position_path_orphaned") and zero($liquidations; "position_conservation_missing") and zero($liquidations; "position_conservation_failures") and zero($liquidations; "deficit_mismatch_instants") and
				(if $extractor_variant == "sv1" then
					zero($liquidations; "execution_summary_failures") and zero($liquidations; "duplicate_execution_summary_records") and zero($liquidations; "duplicate_deficit_records") and zero($liquidations; "missing_liquidation_ids") and zero($liquidations; "unbound_deficit_records") and zero($liquidations; "balance_identity_failures") and zero($liquidations; "deficit_balance_mismatch_accounts")
				else true end)),
