#!/usr/bin/env bash
# Run one immutable V2-4 L1 matched-motive cell. Partial output is never
# evidence and this command refuses to overwrite a prior run directory.
set -euo pipefail
...
arm=$1
seed=$2
case "$arm" in A|B) ;; *) echo "unknown L1 arm: $arm" >&2; exit 2 ;; esac
case "$seed" in 101|103) ;; *) echo "unregistered L1 seed: $seed" >&2; exit 2 ;; esac

root_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
config="$root_dir/research/configs/v2-4-l1/$arm-$seed.json"
output="$root_dir/research/artifacts/v2-4-l1/$arm/seed-$seed"
...
if [[ -e "$output" ]]; then
	echo "refusing to overwrite L1 evidence directory: $output" >&2
	exit 1
fi
...
jq -n \
	...
	'{
	  ...
	  raw_log_policy: "retain until every L1 policy, receipt, execution, fill-transition, exact gap, and non-collapse artifact passes review"
