#!/usr/bin/env bash
# Run one immutable V2-3 P1 inventory-size cell. Existing evidence is never
# overwritten: an interrupted/partial directory is not silently reused.
set -euo pipefail
...
arm=$1
seed=$2
case "$arm" in A|B) ;; *) echo "unknown P1 arm: $arm" >&2; exit 2;; esac
case "$seed" in 101|103) ;; *) echo "unregistered P1 seed: $seed" >&2; exit 2;; esac

root_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
config="$root_dir/research/configs/v2-3-p1/$arm-$seed.json"
output="$root_dir/research/artifacts/v2-3-p1/$arm/seed-$seed"
...
if [[ -e "$output" ]]; then
	echo "refusing to overwrite P1 evidence directory: $output" >&2
	exit 1
fi
...
jq -n \
	...
	'{
	  ...
	  raw_log_policy: "retain until every P1 activation, information-boundary, and viability artifact passes review"
