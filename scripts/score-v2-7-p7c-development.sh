#!/usr/bin/env bash
# Fail-closed development score for the immutable V2-7 P7c screen.
set -euo pipefail

root_dir=$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)
base=${P7C_OUTPUT_ROOT:-"$root_dir/research/artifacts/v2-7-p7c/full"}
output=${P7C_SCORE_OUTPUT:-"$root_dir/research/artifacts/v2-7-p7c/p7c-development-score.json"}
[[ ! -e "$output" ]] || { echo "refusing to overwrite score: $output" >&2; exit 1; }
cells=(C-367 C-371 T-367 T-371)
...
for id in "${cells[@]}"; do
	...
	for f in analysis-metadata.json run-metadata.json run-config.json \
		...
		[[ -s "$cell/$f" ]] || { echo "missing P7c score input: $cell/$f" >&2; exit 1; }
	done
	[[ "$(cat "$cell.extract.status" 2>/dev/null || true)" == 0 ]] || {
		echo "P7c extraction did not pass: $id" >&2
		exit 1
	}
done

source_revision=$(jq -er '.git_revision' "$base/C-367/run-metadata.json")
binary_sha256=$(jq -er '.binary_sha256' "$base/C-367/run-metadata.json")
for id in "${cells[@]}"; do
	cell="$base/$id"
	...
	jq -e --arg rev "$source_revision" --arg bin "$binary_sha256" --arg cfg "$config_sha" \
		'.git_revision == $rev and .binary_sha256 == $bin and .config_sha256 == $cfg and
		 .preflight == false and .simulated_horizon == "48h" and
		 .completion_sentinels == ["greeks.json", "latency.json"]' \
		"$cell/run-metadata.json" >/dev/null
	jq -e '.analysis_contract == "v2-7-p7c-distress-v1" and
	...
	cell_json+=("$(jq -n --arg cell "$arm" --argjson seed "$seed" \
		...
		--slurpfile e "$cell/evidenceartifacthash.json" '
		{
		 ...
		 activation:{enabled_decisions:$p[0].result.enabled_decisions,
		  ...
		  active_valid:($cell=="T" and $p[0].result.enabled_decisions==$p[0].result.decisions and
		 ...
		 risk:{active_mark_checks:$m[0].result.active_mark_checks,
		  ...
		  risk_observed:($actor_margin_events+$actor_liquidations),
...
risk_consistent=$(jq -e 'all(.[];.cell=="C" or
	((.risk.expected_breaches==0 and .risk.risk_observed==0) or
	 (.risk.expected_breaches>0 and .risk.risk_observed>0)))' <<<"$cells_json" >/dev/null && echo true || echo false)
risk_exercised=$(jq -e 'any(.[];.cell=="T" and .risk.expected_breaches>0 and .risk.risk_observed>0)' <<<"$cells_json" >/dev/null && echo true || echo false)
...
if [[ "$integrity" != true || "$control_valid" != true || "$risk_consistent" != true ]]; then
...
jq -n --arg protocol v2-7-p7c-distress-v1 --arg classification "$classification" \
	...
	--argjson integrity "$integrity" --argjson risk_consistent "$risk_consistent" \
	--argjson risk_exercised "$risk_exercised" --argjson deficit_exercised "$deficit_exercised" \
	--argjson cells "$cells_json" '
	{protocol:$protocol,classification:$classification,development_seeds:[367,371],
	 untouched_holdout_seeds:[373,379,383],holdouts_consumed:false,
	 ...
	 predicates:{control_valid:$control_valid,activation_valid:$activation,
	  evidence_integrity_valid:$integrity,risk_event_linkage_consistent:$risk_consistent,
	  actor_risk_path_exercised:$risk_exercised,deficit_path_exercised:$deficit_exercised},
	 verdicts:{fixed_liability_activation:"SUPPORTED (screening)",
	  participant_margin_call:(if $risk_exercised then "SUPPORTED (screening)" else "NOT EXERCISED" end),
	  participant_forced_close:(if $risk_exercised then "SUPPORTED (screening)" else "NOT EXERCISED" end),
	  deficit_insurance_bankruptcy:(if $deficit_exercised then "SUPPORTED (screening)" else "NOT EXERCISED" end)},
	 cells:$cells,
	 interpretation:"P7c scores only the two-day fixed-liability activation and participant-specific risk reachability; generic liquidation totals cannot substitute for a participant-built risk path.",
	 next_action:(if $risk_exercised then "Audit participant-specific risk events before any holdout." else "Do not consume holdouts or tune P7c; retain the NOT EXERCISED result and design a new economic exposure mechanism." end)}' >"$output"
echo "scored P7c development: $classification"
