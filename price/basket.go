...
// venues do: dead sources (returning 0) drop out, survivors are medianed,
// the deviation policy reins in or removes outliers, and the result is the
// weighted average. Fewer than MinSources live constituents — or every
// weight zeroed by the policy — returns 0, which every mark calculator in
// this package already treats as "index unavailable" and handles with its
// own fallback; a degraded basket must never emit a confident bad price.
...
func (b *BasketIndex) Price(symbol string) int64 {
	...
	for _, s := range b.sources {
		...
		if p := s.Source.Price(symbol); p > 0 {
