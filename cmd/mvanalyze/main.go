...
func main() {
	...
	if *metric == "termcarryp4pair" {
		...
		result, err := analysis.MeasureTermCarryP4Pair(control, treatment)
		if err != nil {
			fmt.Fprintf(os.Stderr, "P4 pair: %v\n", err)
			os.Exit(1)
		}
	...
	for _, dir := range flag.Args() {
		...
		case "perpreplenishment":
			...
			if result.Decisions == 0 {
				fmt.Fprintf(os.Stderr, "%s: no P3 perp replenishment decision evidence\n", dir)
				os.Exit(1)
			}
			emit(dir, result, *asJSON, func() {
				fmt.Printf("%-22s decisions %6d enabled/disabled %6d/%6d refresh/no-refresh %6d/%6d accepted/rejected/censored %6d/%6d/%6d lifecycle %6d threshold/lifecycle/outcome/missing %d/%d/%d/%d valid %t\n",
					dir, result.Decisions, result.EnabledDecisions, result.DisabledDecisions, result.RefreshDue, result.NoRefresh,
					result.Accepted, result.Rejected, result.HorizonCensored, result.LifecycleRows,
					result.ThresholdMismatches, result.LifecycleMismatches, result.OutcomeFieldMismatches, result.MissingOutcomes, result.Valid)
			})
		...
		case "priceunavailable":
			result, err := run.MeasurePriceUnavailableOrderRejections()
			if err != nil {
				fmt.Fprintf(os.Stderr, "%s: %v\n", dir, err)
				os.Exit(1)
			}
			emit(dir, result, *asJSON, func() {
				fmt.Printf("%-22s order rejections %d PRICE_UNAVAILABLE %d malformed %d valid %t\n",
					dir, result.OrderRejectedCount, result.PriceUnavailableOrderRejections,
					result.MalformedOrderRejectedCount, result.Valid)
			})
		case "optionliabilityp6":
			...
			emit(dir, result, *asJSON, func() {
				fmt.Printf("%-22s decisions/submits %d/%d defers %d accepted/rejected %d/%d canonical/actor fills %d/%d qty %d future/decode/missing/dup/orphan/mismatch %d/%d/%d/%d/%d/%d target %t valid %t\n",
					dir, result.Decisions, result.SubmitDecisions, result.DeferredDecisions,
		...
		case "datedcarryp5":
			result, err := run.MeasureDatedCarryP5()
			if err != nil {
				fmt.Fprintf(os.Stderr, "%s: %v\n", dir, err)
				os.Exit(1)
			}
			if result.Decisions == 0 {
				fmt.Fprintf(os.Stderr, "%s: no P5 dated-carry decision evidence\n", dir)
				os.Exit(1)
			}
			emit(dir, result, *asJSON, func() {
				fmt.Printf("%-22s trade %t decisions/candidates/eligible %d/%d/%d shadow/targets/submitted %d/%d/%d receipt/roles %t/%t source/frontier/term/arithmetic/policy errors %d/%d/%d/%d/%d valid %t\n",
		...
		case "perpexposurehedger":
			...
			if result.Decisions == 0 {
				fmt.Fprintf(os.Stderr, "%s: no V2-5 P2 perp-exposure decision evidence\n", dir)
				os.Exit(1)
			}
			emit(dir, result, *asJSON, func() {
				fmt.Printf("%-22s decisions %6d enabled/disabled %6d/%6d updates %6d submitted/accepted/rejected %6d/%6d/%6d fills/cancelled %6d/%6d receipt ok/missing/mismatch/future %t/%d/%d/%d fields/outcomes/terminal %d/%d/%d fee/counterparty/self/nonreduce %d/%d/%d/%d valid %t\n",
					dir, result.Decisions, result.EnabledDecisions, result.DisabledDecisions, result.StateUpdates,
					result.Submitted, result.Accepted, result.Rejected, result.Fills, result.CancelledIOC,
					result.ReceiptAuditValid, result.MissingReceipts, result.ReceiptMismatches, result.FutureReceiptUse,
		...
		case "termcarryp4chain":
			...
			emit(dir, result, *asJSON, func() {
				fmt.Printf("%-22s P4 exact-cost decisions %d, submitted %d, valid %t\n", dir, result.DecisionsEvaluated, len(result.SubmittedCandidates), result.Valid)
			})
		...
		case "conservation":
			...
			emit(dir, result, *asJSON, func() {
				...
				if worst, ok := analysis.WorstResidual(result.OptionExpirySystemInstants); ok {
					fmt.Printf("    option expiry: %d instants, system residual after explicit rounding ledger %d at %s %d\n",
						len(result.OptionExpirySystemInstants), worst.Net, worst.VenueID, worst.Timestamp)
				}
		...
		case "calendar":
			result, err := run.MeasureCalendar(analysis.CalendarOptions{})
			if err != nil {
				fmt.Fprintf(os.Stderr, "%s: %v\n", dir, err)
				os.Exit(1)
			}
			emit(dir, result, *asJSON, func() {
				fmt.Printf("%-22s calendar futures %d options %d shared %d listings %d settlements %d duplicates %d/%d\n",
					dir, len(result.FuturesExpiryNanos), len(result.OptionExpiryNanos),
					len(result.SharedExpiryNanos), result.ListingEvents, result.SettlementEvents,
					result.DuplicateListings, result.DuplicateSettlements)
			})
		case "viability":
